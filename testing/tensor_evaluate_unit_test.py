...
subsdict = {Symbol('J'): 1, Symbol("J'"): 1, Symbol('L'): 2, Symbol("L'"): 2, Symbol('l'): 1,
            Symbol("l'"): 1, Symbol('s'): 1, Symbol("s'"): 1, Symbol('j'): 2, Symbol("j'"): 2}
...
class TensorEvaluateTest(unittest.TestCase):
    ...
    def test_evaluate_two_spaces(self):
        ...
        self.assertEqual("<1||{sig1_1 x sig2_1}_0||1>*<p'1||{q_1 x q_1}_0||p1>/3 +"
                         " <1||{sig1_1 x sig2_1}_2||1>*<p'1||{q_1 x q_1}_2||p1>/30", str(result))

    def test_init_matrix_element_three_spaces(self):
        me = MatrixElement(bra_3, ket_3, tensor_op_3)
        self.assertEqual("-2*sqrt(3)/3 * <p'P'J'(j'(l's')L')m_J'|{{{q_1 x q_1}_0 x {sig1_1 x sig2_1}_0}_0 x "
                         ...
                         "{sig1_1 x sig2_1}_2}_2 x {P_1 x P_1}_2}_0|pPJ(j(ls)L)m_J>", str(me))

    def test_matrix_element_full_decouple_three_spaces(self):
        me = MatrixElement(bra_3, ket_3, tensor_op_3)
        composite = me.full_decouple()
        self.assertEqual("-2*(-1)**(J' + L' + j)*sqrt(3)*KroneckerDelta(J, J')/3 * SixJ(j' L' J'; L j 0) * "
    ...
    def test_evaluate_three_spaces(self):
        me = MatrixElement(bra_3, ket_3, tensor_op_3)
        result = me.evaluate(subsdict)
        self.assertEqual("-2*sqrt(15)*<1||{sig1_1 x sig2_1}_0||1>*<P'2||{P_1 x P_1}_0||P2>*<p'1||{q_1 x q_1}_0||p1>/45"
