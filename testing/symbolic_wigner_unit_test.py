...
class TestSymbolicWigner(unittest.TestCase):
    ...
    def test_symbolic_multiplication(self):
        ...
        for j1, j2, j3, j4, j5, j6, j7, j8, j9, j10, j11, j12, j13, j14, j15 in combs:
            if not (abs(j1 - j2) <= j3 <= j1 + j2) or not (abs(j4 - j5) <= j6 <= j4 + j5) \
                    or not (abs(j7 - j8) <= j9 <= j7 + j8) or not (abs(j1 - j4) <= j7 <= j1 + j4) \
                    or not (abs(j2 - j5) <= j8 <= j2 + j5) or not (abs(j3 - j6) <= j9 <= j3 + j6) \
                    or not (abs(j11 - j12) <= j10 <= j11 + j12) or not (abs(j14 - j15) <= j10 <= j14 + j15) \
                    or not (abs(j11 - j15) <= j13 <= j11 + j15) or not (abs(j12 - j14) <= j13 <= j12 + j14):
    ...
    def test_mult_composite(self):
        ...
        for j1, j2, j3, j4, j5, j6, j7, j8, j9, j10, j11, j12, j13, j14, j15, j16, j17, j18 in combs:
            symbolic_comb = symb.evaluate(dict(zip(symbol_list,
                                                   [j1, j2, j3, j4, j5, j6, j7, j8, j9, j10, j11, j12, j13, j14, j15,
                                                    j16, j17, j18])))
            wigner_comb = wigner_6j(j1, j2, j3, j4, j5, j6) * wigner_6j(j7, j8, j9, j10, j11, j12) \
                          * wigner_6j(j13, j14, j15, j16, j17, j18) * 2
            self.assertEqual(wigner_comb, symbolic_comb)
