...
class TestTensorOperator(unittest.TestCase):
    ...
    def test_couple(self):
        ...
        self.assertEqual("-1 * {{k_1 x k_1}_2 x {q_1 x q_1}_2}_1",
                         str(self.basic_block.couple(self.basic_block, 2, 1)
                             .couple((self.other_block.couple(self.other_block, 2, 1)), 1, 1)))
        self.assertEqual("-1 * {{k_1 x q_1}_1 x {q_1 x q_1}_2}_3",
                         str(self.basic_block.couple(self.basic_block, 2, 1)
                             .couple((self.basic_block.couple(self.other_block, 1, 1)), 3, 1)))
        ...
        self.assertEqual("1 * {{q_1 x q_1}_0 x k_1}_1",
                         str(self.basic_block.couple(self.basic_block, 0, 1)
    ...
    def test_couple_tensor_operator_list(self):
        ...
        self.assertEqual("1 * {q_1 x q_1}_0 + 1 * {q_1 x sig_1}_0 + 1 * {k_1 x q_1}_0 + 1 * {k_1 x sig_1}_0",
                         str(tlist.couple(other_tlist, 0, 1)))
        # Coupling two tensor lists with zero elements
        self.assertEqual("1 * {q_1 x sig_1}_1 + 1 * {k_1 x q_1}_1 + 1 * {k_1 x sig_1}_1",
                         str(tlist.couple(other_tlist, 1, 1)))
